...
class App:
    ...
    def draw_curve(self):
        ...
        draw_sierpinski_triangle(
            ...
            x1=x1, y1=offset_y,
            x2=x1 - side / 2, y2=height + offset_y,
            x3=x1 + side / 2, y3=height + offset_y
        )


def draw_sierpinski_triangle(canvas: tk.Canvas, depth: int, x1: float, y1: float, x2: float, y2: float, x3: float, y3: float):
    if depth < 1:
