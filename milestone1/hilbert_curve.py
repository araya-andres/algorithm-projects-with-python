...
def draw_hilbert_curve(canvas: tk.Canvas, depth: int):
    ...
    assert(-lowerLeft.x == upperRight.y) # each curve is contained in a square with area 1
    ...
    for i in range(len(points) - 1):
        canvas.create_line(
            -k * points[i].x + offset_x, k * points[i].y + offset_y,
            -k * points[i + 1].x + offset_x, k * points[i + 1].y + offset_y)
