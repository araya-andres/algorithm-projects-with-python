...
class App:
    ...
    def __init__(self):
        ...
        frame = tk.Frame(outer_frame)
        frame.pack(fill=tk.X, pady=(0,4))
        self.draw_button = tk.Button(frame, text='Draw', width=7, command=self.draw_curve)
...
def draw_branch(canvas: tk.Canvas, depth: int, scale: float, angle1: float, angle2: float, current_angle: float, current_length: float, current_x: float, current_y: float):
    if depth <= 0:
        return
    ...
    draw_branch(canvas, depth - 1, scale, angle1, angle2, current_angle + angle1, scale * current_length, x, y)
    draw_branch(canvas, depth - 1, scale, angle1, angle2, current_angle + angle2, scale * current_length, x, y)
